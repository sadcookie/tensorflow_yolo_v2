...
class Detector(object):
    ...
    def camera_detector(self, cap, wait=1):
        detect_timer = Timer()
        ret, _ = cap.read()
        debugFrame = False
        ...
        while ret:
            ...
            KEY_INPUT = cv2.waitKey(1) # wait
            if KEY_INPUT is ord('q'):
                break
            elif KEY_INPUT is ord('f'):
                if not debugFrame :
                    debugFrame = True
                elif debugFrame :
                    debugFrame = False
            elif KEY_INPUT is ord('p'):
                if not debugProcessTime :
                    debugProcessTime = True
                elif debugProcessTime :
                    debugProcessTime = False

            if debugFrame is True :
                # put frame rate text on the image
                cv2.putText(frame,'FPS : {:.2f}' .format(video_frame_rate)
                    , (10,40), cv2.FONT_HERSHEY_SIMPLEX, 1,(0,0,0), 2 ,cv2.LINE_AA)

            if debugProcessTime is True :
                # put frame rate text on the image
                cv2.putText(frame,'Process Time : {:.2f}' .format(detect_timer.diff)
                    , (10,70), cv2.FONT_HERSHEY_SIMPLEX, 1,(0,0,0), 2 ,cv2.LINE_AA)

            cv2.imshow('Camera', frame)
            frameCount+=1
            # calculate video frame rate
            # print( "Fps : ", 1.0/ (time.time() - start_time))
            video_frame_rate = 1.0/ (time.time() - start_time)
            ret, frame = cap.read()
    ...
    def image_detector(self, imname, wait=0):
        ...
        cv2.putText(image,'Detectig Time : {:.3f} sec' .format(detect_timer.diff)
            , (10,40), cv2.FONT_HERSHEY_SIMPLEX, 1,(0,0,0),1,cv2.LINE_AA)
